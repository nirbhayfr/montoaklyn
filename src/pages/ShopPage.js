...
export const ShopPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (category)
      list = list.filter((p) => (p.category?.name || "Others") === category);
      ...
      <section className="ul-container" style={{ marginTop: 20 }}>
        <div className="ul-toolbar-inline">
          ...
          <div className="ul-toolbar-count">
            Showing <strong>{filtered.length}</strong> / {apiProducts.length}
          </div>
        </div>
      </section>
    ...
    <div className="col">
      ...
      >
        ...
        <div className="px-3 pb-3 mt-2">
          <span className="text-black font-semibold text-base">Rs {price}</span>
        </div>
      </div>
    </div>
  );
